...
def test_get_date(id, year, month, day):
    user = User.objects.get(id=id)
    diary_query = DIARY.objects.filter(USER_ID=user, DATE__year=year, DATE__month=month, DATE__day=day).order_by("-DATE")
    ...
    if detail_query.count() == 0:
        print(f"Internal Error: found {detail_query.count()} DIARY_DETAIL for DIARY({datetime(year, month, day)})")
        return
...
def change_date(id, year, month, day):
    diary_query = DIARY.objects.filter(id=id).order_by("-DATE")
    if diary_query.count() == 0:
        return

    diary = diary_query.first()

    diary.DATE = diary.DATE.replace(year=year, month=month, day=day)
    diary.save()
